...
from task_manager.app.models.tasks import TASKS
...
def test_title_must_contain_between_3_50_characters():
    client = TestClient(app)
    response = client.post("/api/v1/tasks", json={"title": 2 * "*"})

    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    response = client.post("/api/v1/tasks", json={"title": 51 * "*"})
...
def test_post_task_must_have_a_description():
    client = TestClient(app)
    response = client.post("/api/v1/tasks", json={"title": "A title"})
...
def test_description_must_max_length_of_140_characters():
    ...
    response = client.post(
        "/api/v1/tasks", json={"title": "A title", "description": 141 * "*"}
    )
...
def test_post_task_must_be_returned():
    ...
    client = TestClient(app)
    response = client.post("api/v1/tasks", json=task_expected)
...
def test_post_task_must_be_a_unique_id():
    ...
    client = TestClient(app)
    response_1 = client.post("api/v1/tasks", json=task_1)
    response_2 = client.post("api/v1/tasks", json=task_2)
